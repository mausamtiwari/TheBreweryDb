# Intec Brussel Examen 02 : Databases
...
**Voeg je SQL code hieronder:**

- [x] Creëer een gebruiker met de gebruikersnaam 'chef' en met een wachtwoord 'P@ssw0rd' (0 als getal zero)
	CREATE USER 'chef'@'localhost' IDENTIFIED BY 'P@ssw0rd';

- [x] Creëer de database 'TheBreweryDB' :
   
   
   
     CREATE DATABASE TheBreweryDB;
   
     GRANT ALL ON TheBreweryDB.* TO 'mausam'@localhost';
   
      FLUSH PRIVILEGES;
   
   
   
    **1.1 Start met om de tabel 'Brewers' te creëren met de volgende SQL query set:**
...
**1.2 Creëer 2de tabel Categories.**
...
**1.3 Maak de laatste tabel Beers aan.**
...
inner join thebrewerydb.brewers r on b.BrewerId = b.Id
inner join thebrewerydb.categories c on b.CategoryId = b.Id;

```

TIP: gecorrecteerde versie moet meerdere records teruggeven. 
...
**3. Toon alle records van het Beers tabel met de volgende criteria** 
   ...
   - [x] moest eers met de tabel categories samengevoegd worden (gebruik hiervoor inner join)
   - [x] samengevoegde tabel heeft de volgende info:
      1. uniek identifier van de bier,
       de naam van de bier als BierName,
          ...
          versie,
          image
   - [x] moet in de categories van 'Alcoholvrij' OF 'Light' zijn.
   - [x] alle records moet op hun namen 'J' bevatten **_OF_** moet de eerste twee karakters 'St' starten.
   - [x] **_EN_** alcohol procentage moet minder of gelijk aan 2% zijn.
...
**4. Toon alle records van het Beers tabel met de volgende criteria** 
   ...
   - [x] Voeg alle drie tabelen samen. (gebruik hiervoor left join, brewerId mag NULL zijn)
   - [x] Haal niet alle kolommen op! Je moet de volgende kolommen opvragen:
      - [x] uniek sleutel (primary key) van de bier (hernoemd als BierID)
      - [x] naam van de bier (hernoemd als BierName)
      - [x] naam van de brewer (hernoemd als BrewerName)
      - [x] naam van de category (hernoemd als CategoryName)
      - [x] prijs van de bier (hernoemd als BierPrice)
   - [x] filter alle bieren van de brewery 'Boon'



*** (Het einde van de vragen. Jullie zijn al klaar, proficiat !)***
...
**Als je klaar bent, upload je de code als zip-bestand naar "Aankondigingen -> Files -> Cursisten Examens->Exam2". **

**Vergeet niet dat een juiste naamgevingsconventie volgen essentieel is; bijvoorbeeld: "Exam02 Hilal Demir.zip". **


Zorgen ervoor dat jullie het mark-down-bestand (.md) aan de zip hebben toegevoegd. Bovendien, vergeten dan niet om  een **wachtwoord** in te stellen voor het zip-bestand. Probeer het minst 4 karakters te zetten in jullie wachtwoord.**
...
## Het bericht van de instructeur

>> **Begin waar je bent. Gebruik wat je hebt. Doe wat je kan. Veel succes.**
